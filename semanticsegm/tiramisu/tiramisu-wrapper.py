...
def main(job_id, params):
    ...
    print("right after calling main")
    ...
    dropout_prob = params['dropout_prob'][0]
    ...
    x = create_tiramisu(3, img_input,nb_layers_per_block=blocks, p=0.2, wd=weight_decay, growth_rate=16)
    ...
    cp_callback = keras.callbacks.ModelCheckpoint("./spearmint_training/weights-{epoch:02d}-{val_loss:.2f}-" + str(len(blocks)) +"-" + str(time.time())[-7:-3] + "-.hdf5", monitor='val_loss', verbose=2, save_best_only=False, save_weights_only=False, mode='auto', period=1)
    ...
    model.fit(trn, trn_labels.squeeze().reshape(trn.shape[0],image_height*image_width,1), batch_size=batch_size,
    ...
    return model.evaluate(test, test_labels.squeeze().reshape(test.shape[0],image_height*image_width,1))[0]

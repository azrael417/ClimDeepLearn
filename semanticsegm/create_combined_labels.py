...
def plot_mask(lons, lats, img_array, storm_mask, storm_lon, storm_lat,
	...
	print(np.any(storm_mask == 1))
	my_map.contourf(x_map,y_map,np.ma.masked_less(storm_mask,0.5), alpha=0.5,cmap=cmap,vmin=1,vmax=2)
...
for i,table_name in enumerate(teca_subtables[progress_counter:]):
	...
	for time_step_index in range(8):
		...
		with nc.Dataset(path_to_CAM5_files+"{:04d}-{:02d}-{:02d}-00000.nc".format(year, month, day)) as fin:
			...
			lats = fin['lat'][:]
			lons = fin['lon'][:]
			U850 = fin.variables['U850'][:][time_step_index:time_step_index+1]
			V850 = fin.variables['V850'][:][time_step_index:time_step_index+1]
			QREFHT = fin.variables['QREFHT'][:][time_step_index:time_step_index+1]
		...
		if len(curr_table_time) >= 3:
			...
			if len(instance_masks) > 0:
					...
					plot_mask(lons, lats, TMQ.squeeze(), semantic_mask, row['lon'], row['lat'], year, month, day, time_step_index)
					#np.save(path_to_labels+"images_whole/INTERSECT_{:04d}{:02d}{:02d}{:02d}_image.npy".format(year,month,day,time_step_index), TMQ[:,:,np.newaxis])
					#np.save(path_to_labels+"semantic_combined_labels_whole/INTERSECT_{:04d}{:02d}{:02d}{:02d}_semantic_mask.npy".format(year,month,day,time_step_index), semantic_mask)
					...
					#else:
						#np.save(path_to_labels+"images_whole/{:04d}{:02d}{:02d}{:02d}_image.npy".format(year,month,day,time_step_index), TMQ[:,:,np.newaxis])
						#np.save(path_to_labels+"semantic_combined_labels_whole/{:04d}{:02d}{:02d}{:02d}_semantic_mask.npy".format(year,month,day,time_step_index), semantic_mask)
						#instance_labels = [num_instances,np.asarray(instance_boxes), np.asarray(instance_masks)]
						...
						#p.save(path_to_labels+"instance_combined_labels/{:04d}{:02d}{:02d}{:02d}_instance_masks.npy".format(year,month,day,time_step_index), np.asarray(instance_masks))

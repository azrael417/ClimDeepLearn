...
def create_dataset(h5ir, datafilelist, batchsize, num_epochs, comm_size, comm_rank, dtype, shuffle=False):
    ...
    dataset = dataset.map(map_func=lambda dataname: tuple(tf.py_func(h5ir.read, [dataname], [dtype, tf.int32, dtype, tf.string])), num_parallel_calls = 1) #DEBUG should be 4
    #dataset = dataset.map(map_func=lambda dataname: tuple(tf.py_func(h5ir.sequential_read, [dataname], [dtype, tf.int32, dtype, tf.string])), num_parallel_calls = 4)

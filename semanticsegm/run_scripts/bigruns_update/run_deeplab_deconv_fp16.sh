...
APP="python ./deeplab-tf.py --datadir_train ${1}/train/data --datadir_validation ${1}/validation/data --chkpt_dir checkpoint.fp16.lag${5} --epochs ${2} --fs local --loss weighted --cluster_loss_weight 0.0 --optimizer opt_type=LARC-Adam,learning_rate=${3},gradient_lag=${5} --model=resnet_v2_50 --scale_factor ${4} --batch 2 --decoder=deconv1x --dtype float16"

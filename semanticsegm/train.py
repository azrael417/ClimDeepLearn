...
def train():
    ...
    cam5_files = glob.glob("/global/cscratch1/sd/mwehner/CAM5-1-0.25degree_All-Hist_est1_v3_run2/run/h2/CAM5-1-0.25degree_All-Hist_est1_v3_run2.cam.h2.*")
    
    image =[]
    #read in data
    for each_image in cam5_files:
        image += [get_tmq(each_image)]
    image = np.asarray(image)

    ih = 768
    iw = 1152
    gt_boxes = np.load("/global/cscratch1/sd/amahesh/segmentation_labels/instance_combined_labels/instance_boxes.npy")
    gt_masks = np.load("/global/cscratch1/sd/amahesh/segmentation_labels/instance_combined_labels/instance_masks.npy")
    img_id = np.load("/global/cscratch1/sd/amahesh/segmentation_labels/instance_combined_labels/img_ids.npy")
